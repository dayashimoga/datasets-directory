...
class TestCheckUrl:
    ...
    async def test_head_fails_fallback_to_get_fails(self):
        ...
        url, is_valid, error = await check_url(mock_session, "https://test.com")
        assert not is_valid
        assert "GET returned HTTP 404" in error

    async def test_head_fails_no_fallback(self):
        mock_head_resp = AsyncMock()
        mock_head_resp.status = 500
        mock_head_ctx = AsyncMock()
        mock_head_ctx.__aenter__.return_value = mock_head_resp
        mock_head_ctx.__aexit__.return_value = None
    ...
    async def test_client_error(self):
        ...
        url, is_valid, error = await check_url(mock_session, "https://test.com")
        assert not is_valid
        assert "Client error: Test network error" in error
...
class TestMainAsync:
    ...
    async def test_all_links_valid(self, mock_check_url, mock_db_path, mock_db_items, tmp_path):
        ...
        with patch("builtins.open", return_value=db_file.open("r", encoding="utf-8")):
            total_items, total, broken, report = await main_async(fail_fast=False)
    ...
    async def test_broken_links_found(self, mock_check_url, mock_db_path, mock_db_items, tmp_path):
        ...
        with patch("builtins.open", return_value=db_file.open("r", encoding="utf-8")):
            ...
            assert total == 4
            assert broken == 2
            assert "https://broken.com" in report
            assert report["https://broken.com"]["error"] == "HTTP 404"
            assert report["https://broken.com"]["affected_items"] == ["Broken Item"]
...
class TestGenerateReport:
    ...
    def test_clean_report(self):
        report_text = generate_report(total_items=15, total=10, broken=0, broken_links={})
        ...
        assert "Broken Links Details" not in report_text

    def test_error_report(self):
        broken = {"https://bad.com": {"error": "404", "affected_items": ["Item 1"]}}
        report_text = generate_report(total_items=20, total=10, broken=1, broken_links=broken)
        assert "Total Database Items Scanned:** 20" in report_text
        assert "Total Links Checked:** 10" in report_text
        assert "Broken Links:** 1" in report_text
        assert "Broken Links Details" in report_text
        assert "### https://bad.com" in report_text
        assert "**Error**: 404" in report_text
        assert "**Affected Items**: Item 1" in report_text
...
class TestMainCLI:
    ...
    def test_failure_exit_with_report(self, mock_args, mock_async, tmp_path):
        ...
        async def mock_fail(*args, **kwargs):
            return 5, 10, 1, {"https://bad": {"error": "test", "affected_items": []}}
        ...
        assert "Broken Links Details" in report_file.read_text(encoding="utf-8")
