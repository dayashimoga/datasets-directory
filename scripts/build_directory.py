...
def copy_static_assets():
    ...
    for filename in ["ads.txt", "robots.txt"]:
...
def build_item_pages(env: Environment, items: list, categories: dict):
    ...
    for item in items:
        ...
        html = template.render(
            ...
            page_title=f"{item['title']} - Public Dataset | {SITE_NAME}",
...
def build_category_pages(env: Environment, categories: dict):
    ...
    for name, items in categories.items():
        ...
        html = template.render(
            ...
            page_title=f"Free {name} Public Datasets | {SITE_NAME}",
            page_description=f"Browse {len(items)} free public datasets for {name.lower()}. Find the best data for your next project.",
...
def build_index_page(env: Environment, items: list, categories: dict):
    ...
    html = template.render(
        ...
        page_title=f"{SITE_NAME} — Discover Public Datasets",
